 ...
 * Handles reservation form submissions, verifies Turnstile tokens,
 * and sends email notifications using the Resend API.
 * 
...
export default {
  async fetch(request, env, ctx) {
    ...
    // Route only POST requests to /api/book
    if (request.method === "POST" && url.pathname === "/api/book") {
      try {
        ...
        if (!resendApiKey) {
          ...
          // If no Resend API key is configured, return success in development mode but log details
          return new Response(
            JSON.stringify({ 
              success: true, 
        ...
        // Send email to administrative address (contact@prestigevtcstrasbourg.fr)
        // We also CC the client as a confirmation receipt
        const emailRequest = {
        ...
        // If you are using Resend on a custom domain, make sure to configure it. 
        // Note: For Resend free accounts, you can only send to your own registered email address unless domain is verified.
